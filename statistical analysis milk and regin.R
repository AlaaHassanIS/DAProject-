...
system("git init")  # تهيئة Git في المجلد
system("git remote add origin https://github.com/AlaaHassanIS/DAProject-.git")  # ربط المستودع بـ GitHub
system("git add .")  # إضافة جميع الملفات إلى Git
system('git commit -m "إضافة الملفات الأولية"')  # حفظ التغييرات
system("git push -u origin master")

bartlett.test(Milk ~ Region, data = WholeSCutmr)
...
shapiro.test(WholeSCutmr$Fresh)

# Independent t-test: Comparing 'Milk' spending between channels
t.test(Milk ~ Channel, data = WholeSCutmr)

# ANOVA: Comparing 'Grocery' spending across different regions
anova_result <- aov(Grocery ~ as.factor(Region), data = WholeSCutmr)
summary(anova_result)

# Correlation matrix for numerical variables
cor_matrix <- cor(WholeSCutmr[, 3:8])
print(cor_matrix)
